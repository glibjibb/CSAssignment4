...
public interface CoinGameModel {
   ...
   *
   * Additionally checks whether the game is over after a move. If so,
   * return the winner of the game. Increments the player turn number by 1,
   * unless new turn is greater than player array length, in which case set
   * turn to 1.
   *
  ...
  /**
   * Gets the player who has won, once the game is over
   * Assumes the game is over
   *
   * @return the player whose turn it was when game ended
   * @throws IllegalStateException if game is not over
   */
  int getWinner();

  /**
   * Add a new player at the turn order given. If turn is > number of players, insert at end
   *
   * @throws IllegalArgumentException if turn is <= 0;
   */
  void addPlayer(int turn);

  /**
   * Returns the player number in the index of the array
   *
   * @returns player number at index
   * @throws IllegalArgumentException if index is not in players or invalid
   */
